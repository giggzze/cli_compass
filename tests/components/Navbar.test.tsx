...
import { mockClerk } from '@/test/mocks/clerk';
...
test.describe('Navbar Component', () => {
  ...
  test('shows sign in button for unauthenticated users on home page', async ({ page }) => {
  ...
  test('shows public navigation for unauthenticated users on other pages', async ({ page }) => {
    // Mock unauthenticated state
    await mockClerk(page, { authenticated: false });
    ...
    await expect(page.getByRole('link', { name: 'Commands' })).toHaveAttribute('href', '/public/commands');
    await expect(page.getByRole('link', { name: 'Processes' })).toHaveAttribute('href', '/public/process');
  });

  test('shows authenticated navigation on private command page', async ({ page }) => {
    ...
    // Verify correct href attributes
    await expect(page.getByRole('link', { name: 'Add Command' })).toHaveAttribute('href', '/commands/add');
    await expect(page.getByRole('link', { name: 'Processes' })).toHaveAttribute('href', '/process');
  });

  test('shows authenticated navigation on private process page', async ({ page }) => {
    await page.goto('/private/process');
    
    await expect(page.getByRole('link', { name: 'Commands' })).toBeVisible();
    await expect(page.getByRole('link', { name: 'Create Process' })).toBeVisible();
    
    // Verify correct href attributes
    await expect(page.getByRole('link', { name: 'Commands' })).toHaveAttribute('href', '/commands/user');
    await expect(page.getByRole('link', { name: 'Create Process' })).toHaveAttribute('href', '/process/add');
  });
